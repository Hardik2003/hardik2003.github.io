...
featured: true
capabilities:
  - Waste identification confirmed against live usage
  - Rightsizing and Graviton migration analysis
...
There is a large difference between "this instance looks over-provisioned" and "this instance has run below 5% CPU for ninety days". We measure before recommending, and we separate the changes that carry zero risk from the ones that need a change window.

The first pass usually finds the same categories: gateways provisioned for traffic that no longer flows, snapshots from years ago with no lifecycle policy, addresses attached to nothing, and stopped instances still billing for their storage. That work is typically deletable in week one with no code change and no downtime.
...
One-off optimisation decays. Within a year you are back where you started unless something changes structurally.
